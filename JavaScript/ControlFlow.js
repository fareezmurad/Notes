...
function countTruthy(array) {
  ...
  for (let value of array) // to iterate over the items in array
    if (value) // check if value is TRUTHY or FALSEY
...
function showProperties(obj){
  for (let properties in obj)
   if (typeof obj[properties] === 'string') // compare the type of properties of an object. If the properties is STRING, it will show on the console. If not, it will iterate over the function.
...
function sum(limit){
  ...
  for (let i = 0; i <= limit; i++) // Iterate over the value of variable i
    if (i % 3 === 0 || i % 5 === 0) // if i divided by 3 or 5 and not have a remainder, SUM + i. Then iterate over.
...
function showPrimeNumbers(limit) {
  for (let number = 2; number <= limit; number++) // iterate this FOR loop starting from 2 -> limit. If isPrime = true, console will show the all the prime number. if isPrime = false, the number will now show on console.
...
function isPrime(number) {
  ...
  // if the iteration did not find any number with remainder untill the 'limit' value reached, it will return TRUE.
    if (number % factor === 0) 
      return false; // false if iteration find a number with 0 remainder.
